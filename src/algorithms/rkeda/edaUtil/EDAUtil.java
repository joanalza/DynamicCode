 ...
 */
package algorithms.rkeda.edaUtil;
...
public class EDAUtil {
	...
	public static double getRelativePercentageDeviation(rk sol, double opt){
		double val = 0.0;
		val = (sol.fitness - opt)/opt;
		return val;
	}
	
	public static double getPopulationAverageFitness(ArrayList<rk> currentPopulation1){
		double[] vals = new double[currentPopulation1.size()];
		for(int i=0; i<currentPopulation1.size(); i++){
			vals[i] = currentPopulation1.get(i).fitness;
		}
...
//         System.out.println("stdev: "+  stdev);  
      ...
      public static double[]getPMTruncationSelection1(ArrayList<rk> currentPopulation, int truncSize) throws ParseException, IOException {
...
//        ArrayList<rk> currentPopulation = copyPopulation(currentPopulation1);
        ...
        // Sort the population from min to max by the fitness 
        Collections.sort(currentPopulation, new RKCompare());
//        util.sortPopulationRK(currentPopulation);

        // Select the best "truncationSize" solutions of the population
        for (int i = 0; i < truncSize; i++) {
            selectedGenome.add(currentPopulation.get(i).copyOf());
        }
        
        // Calculate the average of the "truncatedSize" elements by their normalized RKs
        double  mean;
        for (int i = 0; i < numberOfActivities; i++) {
            mean = 0;
            ArrayList<Double> S = new ArrayList<Double>();
...
//            }
       ...
       public static double[] getChild(double[] currentModel, double stdev) throws ParseException, IOException {
...
//             double r2 = r.nextGaussian() * currentModel[x][1] + currentModel[x][0];
    ...
    public static rk getBestSolution(ArrayList<rk> Sched, boolean isNatural){
    	if(isNatural){
    		return getBestSolutionMax(Sched);
    	}
    	else{
    		return getBestSolutionMin(Sched);
    	}
    }
...
//        System.out.println("The mean is : "+mean);
	...
	public static int getDiversity (ArrayList<rk> population, int selectionSize, String distance){
		int size = population.size();
		int nBest = selectionSize;
		int totalDiversity = 0;
		if (nBest < 2)
			nBest = size;
		if (distance.contains("cay")){
			for (int i= 0; i<nBest - 1; i++){
				for (int j=i +1 ; j<nBest; j++){
					totalDiversity += ArrayUtils.getCayleyDistance(population.get(i).permutation, population.get(j).permutation);
				}
			}
		}else if (distance.contains("ham")){
			for (int i= 0; i<nBest - 1; i++){
				for (int j=i +1 ; j<nBest; j++){
					totalDiversity += ArrayUtils.getHammingDistance(population.get(i).permutation, population.get(j).permutation);
				}
			}
		}
