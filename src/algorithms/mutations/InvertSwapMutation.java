...
public class InvertSwapMutation implements IPermutationMutation{
	...
	public ArrayList<Integer> mutate(ArrayList<Integer> sol, int distance,
		...
		Random rand2 = new Random();
		int a = rand2.nextInt(sub.length);
		int exchangePosition = a + startPosition;  //select a node from the invert node

		int b = rand2.nextInt(newElements.length);
		...
		for(int i =0; i <newElements.length; i++){
			...
			if (i == b)
				n = newElements[b];	
			if (i== exchangePosition)
				m = newElements[exchangePosition];

		}
		newElements[exchangePosition] = n;
		newElements[b] = m;
